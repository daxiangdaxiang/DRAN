...
# DPGO-second baseline from baselines/DPGO's original multi-robot example.
# It uses selected-robot communication and local RTR optimization.
...
OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/results/dpgo_like_rtr_baseline_six_$(date +%Y%m%d_%H%M%S)}"
...
for dataset in "${DATASET_ORDER[@]}"; do
  ...
  echo "=== DPGO-like RTR baseline ${dataset}: ${rel_path} ==="
  set +e
  DPGO_GRAD_STOP_TOL="${DPGO_GRAD_STOP_TOL}" \
  LD_LIBRARY_PATH="${DPGO_LD_LIBRARY_PATH}:${LD_LIBRARY_PATH:-}" \
    "${BIN}" "${NUM_ROBOTS}" "${data_path}" "${iter_csv}" "${MAX_ITERS}" RTR "${ACCELERATION}" 2>&1 | tee "${log_path}"
