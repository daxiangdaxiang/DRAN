...
void PGOAgent::gradient_tracking_step(double stepsize,int num_iter){
  ...
  for(int cnt=0;cnt<num_iter;cnt++){
    stepsize_r=stepsize;
    for(int cnt_r=0;cnt_r<num_iter;cnt_r++){
        Matrix Xtemp=X; 
      for(auto seperator:localSharedPoseIDs){
        ...
        if(norm_t<1e-5)
            norm_t=1e-5;
        // std::cout<<"normR"<<norm_r<<std::endl;
        // std::cout<<"normt"<<norm_t<<std::endl;

        Xtemp.block(0,index*(d+1),r,d)=R*expmap(-stepsize_r*R.transpose()*gradR/norm_r);
        Xtemp.col(index*(d+1)+d)=(t-stepsize_t*gradt/norm_t);
      }
      //go through shared_neighbor
      for(unsigned i=0;i<shared_neighbor.size();i++){
        unsigned index=num_poses()+i;
        ...
        if(cost_before<cost_after){
          ...
          std::cout<<"stepsize for r and t: "<<stepsize_r<<" , "<<stepsize_t<<std::endl;
    ...
    for(int cnt_r=0;cnt_r<num_iter;cnt_r++){
      ...
      for(unsigned index=0;index<num_poses();index++){
        ...
        if(cost_before<cost_after){
          ...
          flag=true;
          std::cout<<"cost after: "<<cost_after<<std::endl;
          std::cout<<"private stepsize for r and t: "<<stepsize_r<<" , "<<stepsize_t<<std::endl;
